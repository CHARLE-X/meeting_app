...
const EditTemplate: React.FC = () => {
  ...
  const handleAddQuestion = async () => {
    try {
      const response = await fetch(`${API_ENDPOINTS.ADD_QUESTION}/${templateId}?template_name=${templateName}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'ngrok-skip-browser-warning': '69420',
  ...
  const handleDeleteQuestion = async (questionId: number) => {
    try {
      const response = await fetch(`${API_ENDPOINTS.DELETE_QUESTION}/${questionId}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          'ngrok-skip-browser-warning': '69420',
